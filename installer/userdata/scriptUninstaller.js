var product = JSON.parse(
  ng.fs.global().readTextFile(":/userdata/product.json")
);
      ...
      if (
        ng.win32
          ...
          .valueExists(
            ...
            product.platforms.windows.uninstallProductKey,
            false,
            "InstallLocation"
          )
      ) {
        var tempGameLocation = ng.win32
          .registry()
          .value(
            ng.win32.RootKey.HKLM,
            product.platforms.windows.uninstallProductKey,
...
function DoTx() {
  ...
  section.copyFiles("%GAME_EXE%" + ".bak", "%GAME_EXE%");

  section.removeDirectory("%GAME_PATH%/GATE 0");
...
StartPage.prototype.onNext = function () {
  if (ng.systemInfo.platform() == ng.systemInfo.OsFamily.Windows) {
    if (
      checkConflictingProcesses(product.platforms.windows.conflictingProcesses)
    )
      return;
  }
  ...
  if (state.discovery.patchFound) {
    ...
    if (
      ng.fs
        .global()
        .pathIsFile(this.gameDirectoryPicker.value + "/ngreceipt.bin")
    ) {
