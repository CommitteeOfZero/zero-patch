...
function DoTx() {
  ...
  patchContentSection.removeDirectory('%GAME_PATH%/GATE 0');
  patchContentSection.copyFiles('%PATCH_CONTENT%/*', '%GAME_PATH%');
   
  var applyPatchesSection = ng.tx.tx().addSection('Applying patches');
  
  applyPatchesSection.log('Patching game executable...');
  var diffPath = 'laa_patch.vcdiff';
  if (ng.fs.global().pathIsFile(diffPath)
        && ng.fs.global().md5sum('%GAME_EXE%') != product.patchGameExeHash) {
    applyPatchesSection.copyFiles('%GAME_EXE%', '%GAME_EXE%' + '.bak');
    var stream = ng.tx.xdelta3Stream(
        ng.tx.fileStream('%GAME_EXE%' + '.bak'),
        ng.tx.fileStream(diffPath));
    applyPatchesSection.streamOpen(stream);
    applyPatchesSection.writeStream(stream, '%GAME_EXE%');
...
DirectoryPage.prototype.onNext = function() {
  ...
  if (ng.fs.global().md5sum('%GAME_EXE%') != product.origGameExeHash 
       && ng.fs.global().md5sum('%GAME_EXE%') != product.patchGameExeHash) {
    ng.window.messageBox(
        'Game.exe present but invalid. Ensure you are using a fully updated ' +
        product.gameTitle + ' and the latest patch version. If the game has recently received an official update, the patch may have to be updated to match, please check our website.');
    return;
  }
